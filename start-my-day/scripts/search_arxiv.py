...
def calculate_date_windows(target_date: Optional[datetime] = None, days: int = 30) -> Tuple[datetime, datetime, datetime, datetime]:
    ...
    Returns:
        ...
        - window_recent_start: 最近N天窗口开始日期
        - window_recent_end: 最近N天窗口结束日期（即 target_date）
        - window_1y_start: 一年窗口开始日期
        - window_1y_end: 一年窗口结束日期（即 N+1 天前）
    """
    if target_date is None:
        target_date = datetime.now()
    
    # 最近N天窗口: [target_date - N days, target_date]
    window_recent_start = target_date - timedelta(days=days)
    window_recent_end = target_date
    
    # 过去一年窗口（除去最近N天）: [target_date - 365 days, target_date - (N+1) days]
...
def build_config_from_keywords(keywords: List[str], categories: List[str]) -> Dict:
    """
    根据关键词动态构建研究配置（用于 paper-hunt 技能）
...
def main():
    ...
    parser.add_argument('--keywords', type=str, default=None,
                        help='Comma-separated list of keywords (overrides config file, for paper-hunt)')
    ...
    parser.add_argument('--days', type=int, default=30,
                        help='Number of days to search back (default: 30, use 365 for past year)')
    ...
    # 判断使用配置文件还是动态关键词
    if args.keywords:
        # 使用动态关键词（paper-hunt 模式）
        keywords = [k.strip() for k in args.keywords.split(',')]
        logger.info("Using dynamic keywords (paper-hunt mode): %s", keywords)
        config = build_config_from_keywords(keywords, categories)
        logger.info("Built config with %d keywords for categories: %s", len(keywords), categories)
    elif args.config:
        # 使用配置文件（start-my-day 模式）
        logger.info("Loading config from: %s", args.config)
        config = load_research_config(args.config)
    else:
        logger.error("未指定配置文件路径或关键词。请通过 --config 参数、--keywords 参数或 OBSIDIAN_VAULT_PATH 环境变量设置。")
        return 1
    ...
    window_recent_start, window_recent_end, window_1y_start, window_1y_end = calculate_date_windows(target_date, args.days)
    logger.info("Date windows:")
    logger.info("  Recent %d days: %s to %s", args.days, window_recent_start.date(), window_recent_end.date())
    logger.info("  Past year (%d-365 days): %s to %s", args.days+1, window_1y_start.date(), window_1y_end.date())
